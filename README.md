...
<p align="center">
  <img src="https://raw.githubusercontent.com/Nicolai96dk/rf433-sensor-manager/main/custom_components/rf433_sensor_manager/brand/logo.png" alt="RF433 Sensor Manager" width="560">
</p>
...
- Matching guided setup and post-setup Configure screens for the bridge, sensors, and protocol profiles
...
- Bridge-level latest-signal, last-seen, message-count, sensor-count, and unknown-signal diagnostics
...
Protocol setup and Scan/Learn mode stay inside Home Assistant's original setup dialog. The protocol form groups payload positions and event codes into clear sections, shows the latest RF code, and recalculates its interpreted state whenever the code or form values change. The same live profile editor is available later from **Configure → Protocol profiles**. Learning shows only the latest received RF code; the name and door/window type apply to the device ID extracted from that signal. Choose **Add another sensor** to keep scanning or **Done** to save and finish. Areas can be assigned afterward from Home Assistant's device settings. Learned contact sensors start with the open/closed state of their most recent setup signal. Because RF transmissions are unauthenticated, confirm that a displayed code belongs to the intended sensor.

The RF bridge device exposes the latest valid RF signal regardless of whether it belongs to a configured sensor, the time it was received, total traffic statistics, configured sensor count, and distinct unknown-signal count. This makes bridge activity visible directly on the hub device as well as in diagnostics.
